...
uint16_t LCD_X_LENGTH = ILI9341_MORE_PIXEL;
uint16_t LCD_Y_LENGTH = ILI9341_LESS_PIXEL;
...
//	uint8_t ucBuffer [ WIDTH_CH_CHAR*HEIGHT_CH_CHAR/8 ];	
  ...
  GetGBKCode ( ucBuffer, usChar );	
    ...
    /* 取出四个字节的数据，在lcd上即是一个汉字的一行 */
		...
		usTemp = ( usTemp << 8 );
		usTemp |= ucBuffer [ rowCount * 2 + 2 ];
		usTemp = ( usTemp << 8 );
		usTemp |= ucBuffer [ rowCount * 2 + 3 ];
...
void ILI9341_ClearLine(uint16_t Line)
{
...

{
	...
	while ( *ptr != '\0')
	{
		...
		else
		{
				...
				Ascii = *ptr - 32;
				//使用16*32字体缩放字模数据
				ILI9341_zoomChar(16,32,Charwidth,Font_Height,(uint8_t *)&Font16x32.table[Ascii * Font16x32.Height*Font16x32.Width/8],psr,0);
			  //显示单个字符
				ILI9341_DrawChar_Ex(x,y,Charwidth,Font_Height,(uint8_t*)&zoomBuff,DrawModel);
				x+=Charwidth;
				ptr++;
																		 ...
																		 uint16_t DrawModel)  //是否反色显示
{
	...
	{			
		...
		else
		{
				...
				Ascii = *ptr - 32;
				//使用16*24字体缩放字模数据
				ILI9341_zoomChar(16,24,Charwidth,Font_Height,(uint8_t *)&Font16x32.table[Ascii * Font16x32.Height*Font16x32.Width/8],psr,0);
			  //显示单个字符
				ILI9341_DrawChar_Ex(x,y,Charwidth,Font_Height,(uint8_t*)&zoomBuff,DrawModel);
				y+=Font_Height;
				ptr++;
